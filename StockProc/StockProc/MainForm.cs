...
namespace StockProc
{
    public partial class MainForm : Form
    {
        ...
        DataTable[] dtCloned;//Store all the values of each file after escaped
        //DataTable[] dtClonedCopy;
        DataTable dtTablesAllFiles;
        DataTable dtTableMean;
        DataTable[] dtTableMeanLine;
        DataTable[] dtTableMonth;
        DataTable[] dtTableWeek;
        ...
        String[][] StockInfo;//Store the information of each stock
        ...
        private void OpenTXTFileToolStripMenuItem_Click1(object sender, EventArgs e)
        {
            //open Txt files
            this.openFileDialog.Multiselect = true; //Enable select multiple files
                ...
                for (int fi = 0; fi < fileNumber; fi++)
                {
                    try
                    {
                        ...
                        while ((line = file.ReadLine()) != null)
                        {
                            ...
                            {
                                StockInfo[fi] = line.Split((char[])null, StringSplitOptions.RemoveEmptyEntries);
                                LineNumber++;
                                continue;
                            }
                        ...
                        if (year == y)//the same year
                        {
                            if (month == m)// the same month
                            {
                                ...
                                else
                                {
                                    ...
                                    dtTableWeek[fi].Rows[RowIdxCurrent + 1][9] = difTotal;
                                    moneyTotal = 0;
                                    difTotal = 0;
                                    RowIdxCurrent += 2;
